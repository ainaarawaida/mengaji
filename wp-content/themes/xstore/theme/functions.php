    ...
    function etheme_theme_styles() {
        if ( !is_admin() ) {
        	...
        	if ($generated_css){
		        ...
		        // only for wpb
		        if ( is_preview() || get_query_var('et_is_customize_preview', false) ) {
			        ...
			        if ( get_query_var( 'et_mobile-optimization', true ) ) {
				        // back top
			        ...
			        if ( get_query_var( 'is_single_product', false ) ) {
				        ...
				        if ( ( get_query_var( 'etheme_single_product_builder', false ) && etheme_get_option( 'product_navigation_et-desktop', 1 ) ) ||
				             etheme_get_option( 'product_posts_links', 1 ) ) {
					        etheme_enqueue_style( 'navigation' );
				        }
                    ...
                    elseif ( get_query_var( 'et_is-single', false ) ) {
				        ...
				        etheme_enqueue_style( 'star-rating' );
				        etheme_enqueue_style( 'comments' );
				        if ( etheme_get_option( 'posts_links', 1 ) ) {
					        etheme_enqueue_style( 'navigation' );
				        }
...
function etheme_theme_header_styles() {
    ...
    $header        = array();
    $header['top'] = json_decode( get_theme_mod( 'header_' . ( get_query_var( 'is_mobile', false ) ? 'mobile_' : '' ) . 'top_elements', '' ), true );
    if ( ! is_array( $header['top'] ) ) {
        $header['top'] = array();
    }
    uasort( $header['top'], function ( $item1, $item2 ) {
        return $item1['index'] <=> $item2['index'];
    } );
    $header['main'] = json_decode( get_theme_mod( 'header_' . ( get_query_var( 'is_mobile', false ) ? 'mobile_' : '' ) . 'main_elements', '' ), true );
    if ( ! is_array( $header['main'] ) ) {
        $header['main'] = array();
    }
    
    uasort( $header['main'], function ( $item1, $item2 ) {
        return $item1['index'] <=> $item2['index'];
    } );
    $header['bottom'] = json_decode( get_theme_mod( 'header_' . ( get_query_var( 'is_mobile', false ) ? 'mobile_' : '' ) . 'bottom_elements', '' ), true );
    if ( ! is_array( $header['bottom'] ) ) {
        $header['bottom'] = array();
    }
    
    uasort( $header['bottom'], function ( $item1, $item2 ) {
        return $item1['index'] <=> $item2['index'];
    } );
            ...
            if ( $header_part_value['element'] == 'connect_block' ) {
                ...
                if ( $blocks && count( $blocks ) ) {
                    ...
                    if ( $header_part_key !== false ) {
                        $key  = array_search( $header_part_key, array_column( $blocks, 'id' ) );
