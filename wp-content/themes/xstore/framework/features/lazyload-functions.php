...
if ( !function_exists('etheme_placeholder_image2')) {
	function etheme_placeholder_image2( $size = 'woocommerce_thumbnail', $id = '' ) {
	
//		if ( $id != '' ) {
//			$img = etheme_encode_image($id, wp_get_attachment_image_url( $id, $size ));
//			return $img;
//		}
		
		$img = get_theme_mod( 'preloader_images', '' );
		if ( ! empty( $img['url'] ) ) {
			$img = attachment_url_to_postid( $img['url'] );
...
//	    ETHEME_BASE_URI . 'images/lazy' . ( get_theme_mod('dark_styles', 0) ? '-dark' : '' ) . '.png';
	}
}
...
if( ! function_exists( 'etheme_placeholder_image' ) ) {
	function etheme_placeholder_image( $size, $id = false) {
		...
		if ( $placeholder_image ) {
			...
			if ( !image_get_intermediate_size( $placeholder_image, array( absint( $width ), absint( $height ) ) ) ) {
