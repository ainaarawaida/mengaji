...
class WCFMmp_Vendor {
	...
	function wcfmmp_get_vendor_list( $all = false, $offset = '', $number = '', $search = '', $allow_vendors_list = array(), $order = 'ASC', $orderby = 'login', $search_data = array(), $category = '', $has_product = '' ) {
  	...
  	$vendor_arr = array();
		...
		$categoryStudent = "" ;
		$teachingSkills = "" ;
		if(isset($search_data['searchtypengaji']) && $search_data['searchtypengaji'] !== '0' ){
			$luqsearch = true ; 
			if($search_data['searchtypengaji'] === '1'){
				$typengaji = 'guru\_ngaji' ;
			}else{
				$typengaji = 'sekolah\_ngaji';
			}
			
		}

		if(isset($search_data['categoryStudent']) && $search_data['categoryStudent'] !== '0' ){
			$luqsearch = true ; 
			switch ($search_data['categoryStudent']) {
				case '1':
				  $categoryStudent = 'Kanak-kanak 3-6' ;
				  break;
				case '2':
					$categoryStudent = 'Lelaki 7-17' ;
					break;
				case '3':
					$categoryStudent = 'Perempuan 7-17' ;
					break;
				case '4':
					$categoryStudent = 'Lelaki >17' ;
					break;
				case '5':
					$categoryStudent = 'Perempuan >17' ;
					break;
				case '6':
					$categoryStudent = 'Warga Emas >50' ;
					break;
				case '7':
					$categoryStudent = 'Orang Kurang Upaya OKU' ;
					break;
				default:
					$categoryStudent = '' ;
			  } 
		...
		if(isset($search_data['teachingSkills']) && $search_data['teachingSkills'] !== '0' ){
			$luqsearch = true ; 
			$teachingSkills = $search_data['teachingSkills'] ;
			
		}

		if($luqsearch == true){
			if($typengaji != '' && $categoryStudent != '' && $teachingSkills != ''){
				$sql = "SELECT a.user_id, b.`display_name` FROM `{$wpdb->prefix}mengaji` a LEFT JOIN `{$wpdb->prefix}users` b ON a.user_id = b.ID WHERE 
				`type_ngaji` = '{$typengaji}'
				AND
				`ajar_siapa` LIKE '%{$categoryStudent}#%'
				AND
				`kemahiran_mengajar` LIKE '%{$teachingSkills}#%'
				";
			}elseif($typengaji != '' && $categoryStudent != ''){
				$sql = "SELECT a.user_id, b.`display_name` FROM `{$wpdb->prefix}mengaji` a LEFT JOIN `{$wpdb->prefix}users` b ON a.user_id = b.ID WHERE 
				`type_ngaji` = '{$typengaji}'
				AND
				`ajar_siapa` LIKE '%{$categoryStudent}#%'
				";
			}elseif($teachingSkills != '' && $categoryStudent != ''){
				$sql = "SELECT a.user_id, b.`display_name` FROM `{$wpdb->prefix}mengaji` a LEFT JOIN `{$wpdb->prefix}users` b ON a.user_id = b.ID WHERE 
				`ajar_siapa` LIKE '%{$categoryStudent}#%'
				AND
				`kemahiran_mengajar` LIKE '%{$teachingSkills}#%'
				";
			}elseif($teachingSkills != '' && $typengaji != ''){
				$sql = "SELECT a.user_id, b.`display_name` FROM `{$wpdb->prefix}mengaji` a LEFT JOIN `{$wpdb->prefix}users` b ON a.user_id = b.ID WHERE 
				`type_ngaji` = '{$typengaji}'
				AND
				`kemahiran_mengajar` LIKE '%{$teachingSkills}#%'
				...
				`type_ngaji` = '{$typengaji}'
				";
			}elseif($teachingSkills != ''){
				$sql = "SELECT a.user_id, b.`display_name` FROM `{$wpdb->prefix}mengaji` a LEFT JOIN `{$wpdb->prefix}users` b ON a.user_id = b.ID WHERE 
				`kemahiran_mengajar` LIKE '%{$teachingSkills}#%'
				";
			}else{ //categoryStudent != ''
				$sql = "SELECT a.user_id, b.`display_name` FROM `{$wpdb->prefix}mengaji` a LEFT JOIN `{$wpdb->prefix}users` b ON a.user_id = b.ID WHERE 
				`ajar_siapa` LIKE '%{$categoryStudent}#%'
				";
			}
			
			$filtervendor = $wpdb->get_results( $sql );
			$ngajivendor = array(0); 
			foreach( $filtervendor as $all_user ) {
				//$ngajivendor[$all_user->user_id] = esc_attr($all_user->display_name);
				$ngajivendor[] = $all_user->user_id;
			}
			
			$args['include'] = array_merge($args['include'], $ngajivendor);
		}
		//print_r("<pre>");print_r($args);exit();
