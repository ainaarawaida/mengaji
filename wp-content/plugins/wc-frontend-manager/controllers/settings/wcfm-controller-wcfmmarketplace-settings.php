...
class WCFM_Settings_Marketplace_Controller {
	...
	public function processing() {
	  ...
	  // sanitize
		...
		//update background
		if(isset($wcfm_settings_form['type_ngaji'])){
			$tableName = $wpdb->prefix."mengaji" ; 
			$data_update = array('user_id' => $user_id
			,'type_ngaji' => $wpdb->esc_like($wcfm_settings_form['type_ngaji']) 
			,'pengalaman' => $wpdb->esc_like($wcfm_settings_form['pengalaman'])  
			,'ajar_siapa' => $wpdb->esc_like(implode('#',$wcfm_settings_form['ajar_siapa']))  
			,'institusi_pengajian' => $wpdb->esc_like($wcfm_settings_form['institusi_pengajian'])  
			,'kemahiran_mengajar' => $wpdb->esc_like(implode('#',$wcfm_settings_form['kemahiran_mengajar']))  
			,'bahasa_pertuturan' => $wpdb->esc_like(implode('#',$wcfm_settings_form['bahasa_pertuturan']))  
			,'sekolah_ngaji' => $wpdb->esc_like($wcfm_settings_form['sekolah_ngaji'])  
			,'sekolah_pemilik' => $wpdb->esc_like($wcfm_settings_form['sekolah_pemilik'])  
			,'sekolah_nama' => $wpdb->esc_like($wcfm_settings_form['sekolah_nama'])  
			,'sekolah_no_pendaftaran' => $wpdb->esc_like($wcfm_settings_form['sekolah_no_pendaftaran'])  
			,'sekolah_jumlah_guru' => $wpdb->esc_like($wcfm_settings_form['sekolah_jumlah_guru'])  
			,'sekolah_guru_ngaji' => $wpdb->esc_like($wcfm_settings_form['sekolah_guru_ngaji'])  
			,'sekolah_atas_talian' => $wpdb->esc_like($wcfm_settings_form['sekolah_atas_talian'])  
			);
